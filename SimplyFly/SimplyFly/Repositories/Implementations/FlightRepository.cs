...
namespace SimplyFly.Repositories.Implementations
{
    public class FlightRepository : IFlightRepository
    {
        ...
        {
            return await _context.Schedules.Include(s => s.FlightCabins).FirstOrDefaultAsync(s => s.ScheduleId == scheduleId);
        }

        public async Task<IEnumerable<Schedule>> SearchSchedulesAsync(string origin, string destination, DateTime date)
        {
            return await _context.Schedules.Include(s => s.Flight).Include(s => s.FlightCabins)
